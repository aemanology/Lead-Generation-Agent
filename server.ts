...
async function checkWebsitePresence(urlStr: string | undefined): Promise<WebsiteCheck> {
...
function getSocialPlatformName(url: string): string {
...
async function fetchCandidatesFromSERP(
  businessType: string,
  location: string,
...
function deduplicateCandidates(items: CandidateBusiness[]): CandidateBusiness[] {
...
function generateRealisticSerpCandidates(
  businessType: string,
  location: string,
  freelancerService: string,
  count: number
...
// 5. Search Businesses Route
app.post('/api/search-businesses', async (req, res) => {
  try {
    const { businessType, location, freelancerService = 'Web Development', numberOfLeads = 10 } = req.body;
    ...
    const qualifiedList: QualifiedLead[] = [];
    ...
    // (If fewer than requested qualify, return only genuinely qualified leads!)
    ...
    res.status(500).json({ error: err.message || 'Failed to search businesses.' });
  }
});

// 6. Gemini Evidence-Driven Lead Analysis
app.post('/api/analyze-lead', async (req, res) => {
  try {
    const { business, freelancerService } = req.body;
...
Analyze this business strictly using the above evidence. Generate sales outreach details following the JSON schema.`;
    ...
    res.status(500).json({
      ...
      fallbackAnalysis: getEvidenceFallbackAnalysis(req.body.business, req.body.freelancerService)
    });
  }
});
...
function getEvidenceFallbackAnalysis(business: any = {}, service = 'Web Development') {
